...
public class MainFragment extends Fragment implements OnMapReadyCallback {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mlocationListFragment = (LocationListFragment)getActivity().getSupportFragmentManager().findFragmentById(R.id.container_location_list);

        if(mlocationListFragment ==  null)
        {
            mlocationListFragment = LocationListFragment.newInstance();
            getActivity().getSupportFragmentManager().beginTransaction().add(R.id.container_location_list, mlocationListFragment).commit();
        }


        final EditText zipText = (EditText)view.findViewById(R.id.zip_textb);
        zipText.setOnKeyListener(new View.OnKeyListener() {
            @Override
            public boolean onKey(View v, int keyCode, KeyEvent event) {

                if((event.getAction()== KeyEvent.ACTION_DOWN) && keyCode == KeyEvent.KEYCODE_ENTER){
                   // make sure it is a valid zip code
                    ...
                    inm.hideSoftInputFromWindow(zipText.getWindowToken(), 0);
    ...
    public void setUserMarkers(LatLng latLng)
    {
        ...
        if( userMarker ==null)
        {
            ...
            Log.v("DONK", "Current location" + latLng.latitude + "Long: " + latLng.longitude);
        }
    ...
    {
        ArrayList<Devslopes> locations = DataService.getInstance().getBootcampLocationWithin10MiesofZip(zipcode);
